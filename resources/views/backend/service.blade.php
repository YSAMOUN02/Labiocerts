            ...
            <tr>
                ...
                <td>
                    ...
                    <form id="form-{{ $service->id }}" action="/admin/service/update/{{ $service->id }}" method="POST" style="display: inline;">
                        @csrf
                        @method('PUT')
                        <button type="button" class="btn btn-primary update-btn" onclick="showUpdateForm('{{ $service->id }}')"><i class="fa-solid fa-pen-to-square" style="color: #ffffff;"></i></button>
                    </form>
                        ...
                        <div id="update-form-content-{{ $service->id }}" class="modal-content">
                            ...
                            <form id="update-form-{{ $service->id }}" class="form-signin" action="/admin/service/update/{{ $service->id }}" method="POST">
                                ...
                                <select id="status-{{ $service->id }}" class="form-control" name="status" required>
                                    @if($service->status == 0)
                                        <option value="0" {{ $service->status == 'Inactive' ? 'selected' : '' }}>Inactive</option>
                                        <option value="1" {{ $service->status == 'Active' ? 'selected' : '' }}>Active</option>
                                    @else
                                        <option value="1" {{ $service->status == 'Active' ? 'selected' : '' }}>Active</option>
                                        <option value="0" {{ $service->status == 'Inactive' ? 'selected' : '' }}>Inactive</option>
                                    @endif 
                   ...
                   <!-- Delete Button -->
                   <button type="button" class="btn btn-danger delete-btn" onclick="showDeleteConfirm('{{ $service->id }}')"><i class="fa-solid fa-trash" style="color: #ffffff;"></i></button>
