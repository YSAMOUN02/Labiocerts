...
<div class="category-info">
    ...
    <form action="/admin/client/logo/add/submit"  method="POST" enctype="multipart/form-data">
        ...
        <div class="row mt-3">
            <div class="col-12">
                <button class="btn-success"  name="action" value="add" type="submit">Add New</button>
                @if(!empty($preview_logo))
                <button class="btn-primary" name="action" value="update" type="submit">Confirm Update <i class="fa-solid fa-wrench" style="color: #ffffff;"></i></button>
                @endif
            </div>
         </div>
      </div>
  ...
  <table class="mt-2">
    ...
    @foreach ($all_logo as $item)
        <tr>
            ...
            <td>
                <a href="/admin/logoClient/{{$item->id}}"><button class="btn-primary"><i class="fa-solid fa-pen-to-square" style="color: #ffffff;"></i></button></a>
                <button class="btn-danger" onclick="delete_value({{$item->id}} , '#logo_client_id' , '#delete_logo_client')" type="button"><i class="fa-solid fa-trash" style="color: #ffffff;"></i></button>
            </td>
        </tr>
    @endforeach
