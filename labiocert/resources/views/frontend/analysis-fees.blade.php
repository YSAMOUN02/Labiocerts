...
<section>
    @php
    $service4 = $services->where('no', 4)->first();
    @endphp
    @if($service4)
    <div class="labiocert-analysis tenzero container-width">
        <div class="la-text">
            ...
            <ol>
                @foreach(explode("\n", $service4->reference) as $reference)
            ...
            <table class="la-table la-tbl">
                <tbody class="latbl-txt categories">
                    @php
                    $sorted_categories = $services_category->where('service_id', $service4->id)->sortBy('no');
                    ...
                    @endphp
                    @foreach($sorted_parameters as $service_parameter)
